            ...
            <div id="suppr" class="tabcontent create-suppr-bloc" style="display: none;">
                ...
                <ul>
                    ...
                    $query = "SELECT siren, raisonSociale, u.idUser, login
                        FROM CLIENT AS c, UTILISATEUR AS u
                        WHERE c.idUser = u.idUser AND suppr = 0 AND password != 'product' AND type = 'client'
                        ORDER BY idUser;";
                    ...
                    if ($nbLignes > 0) {
                        ...
                        foreach ($lignes as $ligne) {
                            echo "<li><details class='suppr-client clickable'>";
                            echo "<summary>$ligne->raisonSociale</summary>";
                            echo "Login : $ligne->login<br>";
                            echo "N° de SIREN : $ligne->siren<br>";
                            echo "idUser : $ligne->idUser<br>";
                            echo '<br>';
                            // Bouton pour supprimer le client sélectionné
                            echo "<a class='btn-suppr' href='suppr_client.php?id=" . $ligne->idUser . "'>";
                            echo 'Supprimer';
                        ...
                        echo "Aucun client ne peut être supprimé";
                    }
                    ?>
                </ul>
            </div>
